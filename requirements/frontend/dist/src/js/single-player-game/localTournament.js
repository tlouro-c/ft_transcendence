...
export class TournamentGame{
	...
	finish_game(){
		...
		if (this.onFinishCallback) {
			this.onFinishCallback(this.winner);
		}
	}
...
function chooseRandomNames(namesString) {
	// Split the string into an array of names
	const namesArray = namesString.split(", ");
		
	// Check if there are exactly 10 names
		
	// Randomly select 4 names from the array
	const selectedNames = [];
	while (selectedNames.length < 4) {
		const randomIndex = Math.floor(Math.random() * namesArray.length);
		const selectedName = namesArray.splice(randomIndex, 1)[0];
...
export function startLocalTournament(){
	const namesString = "Alice, Bob, Charlie, David, Eva, Frank, Grace, Hannah, Ivan, Jack";
	const randomNames = chooseRandomNames(namesString);
	var player1 = randomNames[0]
	...
	game1.onFinish((winner1) => {
		...
		game2.onFinish((winner2) => {
			...
			game3.onFinish((tournamentWinner) => {
